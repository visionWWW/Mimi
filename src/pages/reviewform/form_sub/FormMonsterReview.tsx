...
const RvForm = (props: any) => {
    const submit = async (values: any) => {
        ...
        try {
            await axios
                .all([
                    axios.post('http://mimi-project.kr:5000/api/review', {grade, explain, restaurant_name, hashtag, user_id})])
    ...
    return (
        <>
            <Navbar></Navbar>
            <h5 style={{textAlign:"center", marginTop:"30px"}}>몬스터플레이스 | 음식점은 어떠셨나요?</h5>
            {/*{getData()}*/}
            <Formik
                initialValues={{
                    grade: '', hashtag: [],
                    explain:'', restaurant_name:"몬스터플레이스", user_id: localStorage.getItem("id")
                }}
                onSubmit={submit}
